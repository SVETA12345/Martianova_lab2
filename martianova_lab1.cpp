...
Station LoadStation(){
    ...
    if (fin.is_open()){
        while(getline(fin, st)){ // пока не достигнут конец файла класть очередную строку в переменную (st)
        ...
        while ( getline( ss, s, ',' ) ) {
            if (index==0){
                ...
                a1[index]=s.substr(9,strlen(s.c_str()));
            }
            else{
                a1[index]=s;
            }
    ...
    station.countAll=stoi(a1[1]);
    station.countWork=stoi(a1[2]);
    station.effect=a1[3];
        }
    }
        fin.close();
